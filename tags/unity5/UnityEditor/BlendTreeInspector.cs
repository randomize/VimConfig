namespace UnityEditor
{
    ...
    internal class BlendTreeInspector : Editor
    {
        ...
        {
            this.m_BlendTree.CreateBlendTreeChild((float) 0f).blendParameter = this.m_BlendTree.blendParameter;
            this.SetNewThresholdAndPosition(this.m_BlendTree.children.Length - 1);
        ...
        public void DrawChild(Rect r, int index, bool isActive, bool isFocused)
        {
            ...
            if (this.m_BlendType.intValue == 0)
            {
                ...
                EditorGUI.BeginChangeCheck();
                string s = EditorGUI.DelayedTextField(rowRects[num], floatValue.ToString(), "inftynaeINFTYNAE0123456789.,-", EditorStyles.textField);
                num++;
            ...
            else
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    float num4;
                    EditorGUI.BeginChangeCheck();
                    string str3 = EditorGUI.DelayedTextField(rowRects[num], vector[i].ToString(), "inftynaeINFTYNAE0123456789.,-", EditorStyles.textField);
                    num++;

    ...
    internal class DefaultPackerPolicy : IPackerPolicy
    {
        ...
        public void OnGroupAtlases(BuildTarget target, PackerJob job, int[] textureImporterInstanceIDs)
        {
            ...
            foreach (int num in textureImporterInstanceIDs)
            {
                ...
                foreach (Sprite sprite in AssetDatabase.LoadAllAssetRepresentationsAtPath(assetToUnload.assetPath).Select<UnityEngine.Object, Sprite>(<>f__am$cache0).Where<Sprite>(<>f__am$cache1).ToArray<Sprite>())
                {
                    ...
                    item.settings.filterMode = !Enum.IsDefined(typeof(UnityEngine.FilterMode), assetToUnload.filterMode) ? UnityEngine.FilterMode.Bilinear : assetToUnload.filterMode;

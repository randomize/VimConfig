namespace UnityEditor.Sprites
{
    ...
    internal class PackerWindow : SpriteUtilityWindow
    {
        ...
        private void OnSelectionChange()
        {
            if (Selection.activeObject != null)
            {
                ...
                if (activeObject != this.m_SelectedSprite)
                {
                    ...
                    {
                        <OnSelectionChange>c__AnonStorey9F storeyf = new <OnSelectionChange>c__AnonStorey9F();
                        Packer.GetAtlasDataForSprite(activeObject, out storeyf.selAtlasName, out storeyf.selAtlasTexture);
                        int num = this.m_AtlasNames.ToList<string>().FindIndex(new Predicate<string>(storeyf.<>m__1E1));
                        if (num == -1)
                        {
                            return;
                        }
                        int num2 = Packer.GetTexturesForAtlas(storeyf.selAtlasName).ToList<Texture2D>().FindIndex(new Predicate<Texture2D>(storeyf.<>m__1E2));
                        if (num2 == -1)
                        {
                            return;
                        }
        ...
        [CompilerGenerated]
        private sealed class <OnSelectionChange>c__AnonStorey9F
        {
            internal string selAtlasName;
            internal Texture2D selAtlasTexture;

            internal bool <>m__1E1(string s)
            {
                return (this.selAtlasName == s);
            }

            internal bool <>m__1E2(Texture2D t)
            {

namespace UnityEditor
{
    ...
    internal class AssetStoreInstaBuyWindow : EditorWindow
    {
        ...
        {
            <DownloadPackage>c__AnonStorey4F storeyf = new <DownloadPackage>c__AnonStorey4F {
                <>f__this = this,
                item = this.m_Asset.previewInfo
            };
            this.m_Purchasing = PurchaseStatus.Downloading;
            storeyf.item.downloadProgress = 0f;
            storeyf.item.buildProgress = -1f;
            AssetStoreContext.Download(this.m_Asset.packageID.ToString(), storeyf.item.packageUrl, storeyf.item.encryptionKey, storeyf.item.packageName, storeyf.item.publisherName, storeyf.item.categoryName, new AssetStoreUtils.DownloadDoneCallback(storeyf.<>m__84));
        }
        ...
        [CompilerGenerated]
        private sealed class <DownloadPackage>c__AnonStorey4F
        {
            internal AssetStoreInstaBuyWindow <>f__this;
            internal AssetStoreAsset.PreviewInfo item;

            internal void <>m__84(string id, string status, int bytes, int total)
            {

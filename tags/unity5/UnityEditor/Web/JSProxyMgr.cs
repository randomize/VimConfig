namespace UnityEditor.Web
{
    ...
    internal class JSProxyMgr
    {
        ...
        private bool DoGetStubInfoMessage(long messageID, ExecCallback callback, Dictionary<string, object> jsonData)
        {
            ...
            foreach (System.Reflection.FieldInfo info5 in destinationObject.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance).ToList<System.Reflection.FieldInfo>())
            {
        ...
        {
            <DoInvokeMessage>c__AnonStorey91 storey = new <DoInvokeMessage>c__AnonStorey91 {
                callback = callback,
                messageID = messageID
            };
            if ((!jsonData.ContainsKey("destination") || !jsonData.ContainsKey("method")) || !jsonData.ContainsKey("params"))
            {
                storey.callback(FormatError(storey.messageID, -1001, "errUnknownObject", "object reference, method name or parameters missing"));
                return false;
            }
            ...
            storey.destObject = this.GetDestinationObject(reference);
            if (storey.destObject == null)
            {
                storey.callback(FormatError(storey.messageID, -1001, "errUnknownObject", "cannot find object with reference <" + reference + ">"));
                return false;
            }
            MethodInfo[] methods = storey.destObject.GetType().GetMethods();
            storey.foundMethod = null;
            storey.parameters = null;
            ...
            foreach (MethodInfo info in methods)
            {
                if (info.Name == str2)
                {
                    try
                    {
                        storey.parameters = this.ParseParams(info, data);
                        storey.foundMethod = info;
                        break;
                    }
            ...
            if (storey.foundMethod == null)
            {
                string[] textArray1 = new string[] { "cannot find method <", str2, "> for object <", reference, ">, reason:", message };
                storey.callback(FormatError(storey.messageID, -1002, "errUnknownMethod", string.Concat(textArray1)));
                return false;
            }
            this.AddTask(new TaskCallback(storey.<>m__1A6));
            return true;
        }
        ...
        private object InternalParseParam(System.Type type, object data)
        {
            ...
            if (dictionary != null)
            {
                ...
                foreach (System.Reflection.FieldInfo info2 in type.GetFields(BindingFlags.Public | BindingFlags.Instance).ToList<System.Reflection.FieldInfo>())
                {
                    try
                    {
        ...
        [CompilerGenerated]
        private sealed class <DoInvokeMessage>c__AnonStorey91
        {

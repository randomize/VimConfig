namespace UnityEditor
{
    ...
    {
        [CompilerGenerated]
        private static Comparison<Rect> <>f__am$cacheF;
        ...
        private void DoApply(SerializedObject so)
        {
            if (this.multipleSprites)
            {
                ...
                for (int i = 0; i < this.m_RectsCache.Count; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(rect.m_Name))
                    {
                        rect.m_Name = "Empty";
                    }
                    if (!string.IsNullOrEmpty(rect.m_OriginalName))
                    {
                        list.Add(rect.m_OriginalName);
                        list2.Add(rect.m_Name);
                    }
                    property.InsertArrayElementAtIndex(i);
                    SerializedProperty arrayElementAtIndex = property.GetArrayElementAtIndex(i);
                    arrayElementAtIndex.FindPropertyRelative("m_Rect").rectValue = rect.m_Rect;
                    arrayElementAtIndex.FindPropertyRelative("m_Border").vector4Value = rect.m_Border;
                    arrayElementAtIndex.FindPropertyRelative("m_Name").stringValue = rect.m_Name;
                    arrayElementAtIndex.FindPropertyRelative("m_Alignment").intValue = (int) rect.m_Alignment;
                    arrayElementAtIndex.FindPropertyRelative("m_Pivot").vector2Value = rect.m_Pivot;
                }
                if (list.Count > 0)
                {

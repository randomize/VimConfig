namespace UnityEditor
{
    ...
    internal class PolygonCollider2DEditor : Collider2DEditorBase
    {
        ...
        [CompilerGenerated]
        private static Func<UnityEngine.Object, PolygonCollider2D> <>f__am$cache3;
        ...
        private void HandleDragAndDrop()
        {
            if ((Event.current.type == EventType.DragPerform) || (Event.current.type == EventType.DragUpdated))
            {
                ...
                IEnumerator<UnityEngine.Object> enumerator = DragAndDrop.objectReferences.Where<UnityEngine.Object>(<>f__am$cache2).GetEnumerator();
                try
                {
                    while (enumerator.MoveNext())
                    {
                        ...
                        {
                            Sprite sprite = !(current is Sprite) ? UnityEditor.SpriteUtility.TextureToSprite(current as Texture2D) : (current as Sprite);
                            if (<>f__am$cache3 == null)
                            {
                                <>f__am$cache3 = target => target as PolygonCollider2D;
                            }
                            IEnumerator<PolygonCollider2D> enumerator2 = base.targets.Select<UnityEngine.Object, PolygonCollider2D>(<>f__am$cache3).GetEnumerator();
                            try
                            {

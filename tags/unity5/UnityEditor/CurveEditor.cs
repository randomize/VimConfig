namespace UnityEditor
{
    ...
    {
        [CompilerGenerated]
        private static Func<CurveWrapper, int> <>f__am$cache25;
        [CompilerGenerated]
        private static Func<CurveWrapper, int> <>f__am$cache26;
        [CompilerGenerated]
        private static Func<CurveSelection, float> <>f__am$cache28;
        [CompilerGenerated]
            ...
            switch (current.type)
            {
                ...
                case EventType.ExecuteCommand:
                {
                    ...
                    if (commandName != null)
                    {
                        ...
                        if (<>f__switch$mapA.TryGetValue(commandName, out num))
                        {
                            switch (num)
                            {
                                case 0:
        ...
        private void EditSelectedPoints()
        {
            ...
            if (this.editingPoints)
            {
                ...
                {
                    <>f__am$cache2C = x => x.keyframe.time;
                }
                float num3 = this.PointFieldForSelection(new Rect(vector.x, vector.y, 80f, 18f), 1, <>f__am$cache2C, "time");
                if (EditorGUI.EndChangeCheck())
                {
                ...
                {
                    <>f__am$cache2D = x => x.keyframe.value;
                }
                float y = this.PointFieldForSelection(new Rect(vector.x, vector.y + 18f, 80f, 18f), 2, <>f__am$cache2D, "value");
                if (EditorGUI.EndChangeCheck())
                {
                ...
                case EventType.MouseDrag:
                {
                    ...
                    if (this.m_DraggingCurveOrRegion == null)
                    {
                        if (current.shift)
                        {
                            ...
                            if (introduced20 > Mathf.Abs(lhs.y))
                            {
                                lhs.y = 0f;
                                vector3 = (Vector3) base.ViewToDrawingTransformVector(lhs);
                                float introduced21 = this.SnapTime(vector3.x + this.s_StartKeyDragPosition.x);
                                vector3.x = introduced21 - this.s_StartKeyDragPosition.x;
                            }
                            else
                            {
                                lhs.x = 0f;
                                vector3 = (Vector3) base.ViewToDrawingTransformVector(lhs);
                                float introduced22 = this.SnapValue(vector3.y + this.s_StartKeyDragPosition.y);
                                vector3.y = introduced22 - this.s_StartKeyDragPosition.y;
                            }
                        }
                        else
                        {
                            vector3 = (Vector3) base.ViewToDrawingTransformVector(lhs);
                            float introduced23 = this.SnapTime(vector3.x + this.s_StartKeyDragPosition.x);
                            vector3.x = introduced23 - this.s_StartKeyDragPosition.x;
                            float introduced24 = this.SnapValue(vector3.y + this.s_StartKeyDragPosition.y);
                            vector3.y = introduced24 - this.s_StartKeyDragPosition.y;
                        }
                    }
                    else
                    {
                        lhs.x = 0f;
                        vector3 = (Vector3) base.ViewToDrawingTransformVector(lhs);
                        float introduced19 = this.SnapValue(vector3.y + this.s_StartKeyDragPosition.y);
                        vector3.y = introduced19 - this.s_StartKeyDragPosition.y;
                    }
                    this.TranslateSelectedKeys(vector3);
                    GUI.changed = true;
                    current.Use();
                    return vector3;
                }
        ...
        {
            <PointFieldForSelection>c__AnonStorey41 storey = new <PointFieldForSelection>c__AnonStorey41 {
                memberGetter = memberGetter,
                <>f__this = this
            };
            float num = 0f;
            if (this.selectedCurves.All<CurveSelection>(new Func<CurveSelection, bool>(storey.<>m__6B)))
            {
        ...
        private void SelectPoints()
        {
            ...
            switch (current.GetTypeForControl(controlID))
            {
                case EventType.MouseDown:
                {
                        ...
                        if (current.button == 0)
                        {
                            ...
                            else
                            {
                                ...
                                if (shift)
                                {
                                    ...
                                    {
                                        <SelectPoints>c__AnonStorey3F storeyf = new <SelectPoints>c__AnonStorey3F {
                                        ...
                                        <SelectPoints>c__AnonStorey40 storey = new <SelectPoints>c__AnonStorey40 {
                                            <>f__ref$63 = storeyf,
                                            keyIndex = num3
                                        };
                                        while (storey.keyIndex <= num4)
                                        {
                                            if (!this.selectedCurves.Any<CurveSelection>(new Func<CurveSelection, bool>(storey.<>m__64)))
                                            {
                                                CurveSelection curveSelection = new CurveSelection(storeyf.rangeCurveID, this, storey.keyIndex);
                                                this.AddSelection(curveSelection);
                                            }
                                            storey.keyIndex++;
                                        }
                    ...
                    <SelectPoints>c__AnonStorey3D storeyd = new <SelectPoints>c__AnonStorey3D {
                        selectedPoint = this.FindNearest()
                    };
                    if (storeyd.selectedPoint == null)
                    {
                    ...
                    else
                    {
                        ...
                        <SelectPoints>c__AnonStorey3E storeye = new <SelectPoints>c__AnonStorey3E {
                            <>f__ref$61 = storeyd,
                            keyIndex = 0
                        };
                        while (storeye.keyIndex < storeyd.selectedPoint.curve.keys.Length)
                        {
                            if (!this.selectedCurves.Any<CurveSelection>(new Func<CurveSelection, bool>(storeye.<>m__63)))
                            {
                                CurveSelection selection = new CurveSelection(storeyd.selectedPoint.curveID, this, storeye.keyIndex);
        ...
        {
            <SetSelectedKeyPositions>c__AnonStorey3C storeyc = new <SetSelectedKeyPositions>c__AnonStorey3C {
                updateTime = updateTime,
                newPosition = newPosition,
                updateValue = updateValue,
                <>f__this = this
            };
            this.UpdateCurvesFromPoints(new Action<SavedCurve.SavedKeyFrame, SavedCurve>(storeyc.<>m__62));
        }
        ...
        private void StartEditingSelectedPoints()
        {
            ...
            {
                <>f__am$cache28 = x => x.keyframe.time;
            }
            float num = this.selectedCurves.Min<CurveSelection>(<>f__am$cache28);
            if (<>f__am$cache29 == null)
            {
                <>f__am$cache29 = x => x.keyframe.time;
            }
            float num2 = this.selectedCurves.Max<CurveSelection>(<>f__am$cache29);
            if (<>f__am$cache2A == null)
            {
                <>f__am$cache2A = x => x.keyframe.value;
            }
            float num3 = this.selectedCurves.Min<CurveSelection>(<>f__am$cache2A);
            if (<>f__am$cache2B == null)
            {
                <>f__am$cache2B = x => x.keyframe.value;
            }
            float num4 = this.selectedCurves.Max<CurveSelection>(<>f__am$cache2B);
        ...
        private void SyncDrawOrder()
        {
            if (this.m_DrawOrder.Count == 0)
            {
                ...
                {
                    <>f__am$cache25 = cw => cw.id;
                }
                this.m_DrawOrder = this.m_AnimationCurves.Select<CurveWrapper, int>(<>f__am$cache25).ToList<int>();
            }
            else
            {
        ...
        {
            <TranslateSelectedKeys>c__AnonStorey3B storeyb = new <TranslateSelectedKeys>c__AnonStorey3B {
                movement = movement,
                <>f__this = this
            };
            this.UpdateCurvesFromPoints(new Action<SavedCurve.SavedKeyFrame, SavedCurve>(storeyb.<>m__61));
        }
        ...
        [CompilerGenerated]
        private sealed class <PointFieldForSelection>c__AnonStorey41
        {
            internal CurveEditor <>f__this;
            internal Func<CurveSelection, float> memberGetter;

            internal bool <>m__6B(CurveSelection x)
            {
        ...
        [CompilerGenerated]
        private sealed class <SelectPoints>c__AnonStorey3D
        {
            internal CurveSelection selectedPoint;
        }

        [CompilerGenerated]
        private sealed class <SelectPoints>c__AnonStorey3E
        {
            internal CurveEditor.<SelectPoints>c__AnonStorey3D <>f__ref$61;
            internal int keyIndex;

            internal bool <>m__63(CurveSelection x)
            {
                return ((x.curveID == this.<>f__ref$61.selectedPoint.curveID) && (x.key == this.keyIndex));
            }
        }

        [CompilerGenerated]
        private sealed class <SelectPoints>c__AnonStorey3F
        {
            internal int rangeCurveID;
        }

        [CompilerGenerated]
        private sealed class <SelectPoints>c__AnonStorey40
        {
            internal CurveEditor.<SelectPoints>c__AnonStorey3F <>f__ref$63;
            internal int keyIndex;

            internal bool <>m__64(CurveSelection x)
            {
                return ((x.curveID == this.<>f__ref$63.rangeCurveID) && (x.key == this.keyIndex));
            }
        }

        [CompilerGenerated]
        private sealed class <SetSelectedKeyPositions>c__AnonStorey3C
        {
            internal CurveEditor <>f__this;
            ...
            internal void <>m__62(CurveEditor.SavedCurve.SavedKeyFrame keyframe, CurveEditor.SavedCurve curve)
            {
                if (this.updateTime)
                {
        ...
        [CompilerGenerated]
        private sealed class <TranslateSelectedKeys>c__AnonStorey3B
        {
            internal CurveEditor <>f__this;
            internal Vector2 movement;

            internal void <>m__61(CurveEditor.SavedCurve.SavedKeyFrame keyframe, CurveEditor.SavedCurve curve)
            {

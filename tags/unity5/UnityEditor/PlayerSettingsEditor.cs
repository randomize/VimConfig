namespace UnityEditor
{
    ...
    internal class PlayerSettingsEditor : Editor
    {
        ...
        private SerializedProperty m_FirstStreamedSceneWithResources;
        ...
        private void ApplyChangedGraphicsAPIList(BuildTarget target, GraphicsDeviceType[] apis, bool firstEntryChanged)
        {
            ...
            {
                flag = false;
                if (EditorUtility.DisplayDialog("Changing editor graphics device", "Changing active graphics API requires reloading all graphics objects, it might take a while", "Apply", "Cancel") && EditorApplication.SaveCurrentSceneIfUserWantsTo())
                {
                    flag = true;
                    flag2 = true;
                }
        ...
        public static void BuildDisabledEnumPopup(GUIContent selected, string uiString)
        {
            ...
            EditorGUI.Popup(EditorGUILayout.GetControlRect(true, new GUILayoutOption[0]), EditorGUIUtility.TextContent(uiString), 0, displayedOptions);
            EditorGUI.EndDisabledGroup();
        }

        public static int BuildEnumPopup<T>(string uiString, int selected, T[] options, GUIContent[] optionNames)
        {
            T local = (T) selected;
            int selectedIndex = 0;
            ...
            int index = EditorGUILayout.Popup(EditorGUIUtility.TextContent(uiString), selectedIndex, optionNames, new GUILayoutOption[0]);
            return (int) options[index];
        }

        public static void BuildEnumPopup<T>(SerializedProperty prop, string uiString, string textPrefixOptional, T[] options)
        {
            int intValue = prop.intValue;
            GUIContent[] optionNamesFromValues = GetOptionNamesFromValues<T>(textPrefixOptional, options);
            int num2 = BuildEnumPopup<T>(uiString, intValue, options, optionNamesFromValues);
            if (num2 != intValue)
            {
                prop.intValue = num2;
        ...
        public static int BuildEnumPopup<T>(string uiString, BuildTargetGroup targetGroup, string propertyName, string textPrefixOptional, T[] options)
        {
            int num = 0;
            ...
            GUIContent[] optionNamesFromValues = GetOptionNamesFromValues<T>(textPrefixOptional, options);
            return BuildEnumPopup<T>(uiString, num, options, optionNamesFromValues);
        }
        ...
        private static GUIContent[] GetOptionNamesFromValues<T>(string textPrefixOptional, T[] options)
        {
            GUIContent[] contentArray = new GUIContent[options.Length];
            for (int i = 0; i < options.Length; i++)
            {
                contentArray[i] = (textPrefixOptional != null) ? EditorGUIUtility.TextContent(string.Format("{0}.{1}", textPrefixOptional, (int) options[i])) : new GUIContent(options[i].ToString());
            }
            return contentArray;
        }
        ...
        {
            <GraphicsAPIsGUIOnePlatform>c__AnonStorey8F storeyf = new <GraphicsAPIsGUIOnePlatform>c__AnonStorey8F {
                targetPlatform = targetPlatform,
                <>f__this = this
            };
            GraphicsDeviceType[] supportedGraphicsAPIs = PlayerSettings.GetSupportedGraphicsAPIs(storeyf.targetPlatform);
            if ((supportedGraphicsAPIs != null) && (supportedGraphicsAPIs.Length >= 2))
            {
                EditorGUI.BeginChangeCheck();
                bool useDefaultGraphicsAPIs = PlayerSettings.GetUseDefaultGraphicsAPIs(storeyf.targetPlatform);
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    PlayerSettings.SetUseDefaultGraphicsAPIs(storeyf.targetPlatform, useDefaultGraphicsAPIs);
                }
                if (!useDefaultGraphicsAPIs)
                {
                    <GraphicsAPIsGUIOnePlatform>c__AnonStorey90 storey = new <GraphicsAPIsGUIOnePlatform>c__AnonStorey90 {
                        <>f__ref$143 = storeyf,
                        <>f__this = this
                    };
                    if (WillEditorUseFirstGraphicsAPI(storeyf.targetPlatform))
                    {
                        EditorGUILayout.HelpBox("Reordering the list will switch editor to the first available platform", MessageType.Info, true);
                    }
                    storey.displayTitle = "Graphics APIs";
                    if (platformTitle != null)
                    {
                        storey.displayTitle = storey.displayTitle + platformTitle;
                    }
                    if (!this.m_GraphicsDeviceLists.ContainsKey(storeyf.targetPlatform))
                    {
                        GraphicsDeviceType[] graphicsAPIs = PlayerSettings.GetGraphicsAPIs(storeyf.targetPlatform);
                        ...
                        ReorderableList list2 = new ReorderableList(elements, typeof(GraphicsDeviceType), true, true, true, true) {
                            onAddDropdownCallback = new ReorderableList.AddDropdownCallbackDelegate(storey.<>m__1A0),
                            onCanRemoveCallback = new ReorderableList.CanRemoveCallbackDelegate(this.CanRemoveGraphicsDeviceElement),
                            onRemoveCallback = new ReorderableList.RemoveCallbackDelegate(storey.<>m__1A1),
                            onReorderCallback = new ReorderableList.ReorderCallbackDelegate(storey.<>m__1A2),
                            drawElementCallback = new ReorderableList.ElementCallbackDelegate(storey.<>m__1A3),
                            drawHeaderCallback = new ReorderableList.HeaderCallbackDelegate(storey.<>m__1A4),
                            elementHeight = 16f
                        };
                        this.m_GraphicsDeviceLists.Add(storeyf.targetPlatform, list2);
                    }
                    this.m_GraphicsDeviceLists[storeyf.targetPlatform].DoLayoutList();
                    this.OpenGLES31OptionsGUI(targetGroup, storeyf.targetPlatform);
                }
            }
        }
            ...
            if (this.BeginSettingsBox(1, EditorGUIUtility.TextContent("Icon")))
            {
                ...
                else if (targetGroup != BuildTargetGroup.Metro)
                {
                    ...
                    for (int i = 0; i < iconSizesForPlatform.Length; i++)
                    {
                        ...
                        if (flag3)
                        {
                            iconsForPlatform[i] = (Texture2D) EditorGUI.ObjectField(new Rect((((rect.x + num3) - 96f) - 64f) - 6f, rect.y, 64f, 64f), iconsForPlatform[i], typeof(Texture2D), false);
                        }
        ...
        private void OnEnable()
        {
            ...
            this.m_FirstStreamedSceneWithResources = this.FindPropertyAssert("firstStreamedSceneWithResources");
        ...
        public void OtherSectionGUI(BuildPlayerWindow.BuildPlatform platform, BuildTargetGroup targetGroup, ISettingEditorExtension settingsExtension)
        {
            ...
            if (this.BeginSettingsBox(4, EditorGUIUtility.TextContent("Other Settings")))
            {
                ...
                if ((((targetGroup == BuildTargetGroup.XBOX360) || (targetGroup == BuildTargetGroup.WiiU)) || ((targetGroup == BuildTargetGroup.Standalone) || (targetGroup == BuildTargetGroup.WebPlayer))) || ((((targetGroup == BuildTargetGroup.iPhone) || (targetGroup == BuildTargetGroup.Android)) || ((targetGroup == BuildTargetGroup.PSP2) || (targetGroup == BuildTargetGroup.PS4))) || ((targetGroup == BuildTargetGroup.PSM) || (targetGroup == BuildTargetGroup.Metro))))
                {
                    EditorGUI.BeginChangeCheck();
                ...
                if (((targetGroup == BuildTargetGroup.Standalone) || (targetGroup == BuildTargetGroup.Android)) || (targetGroup == BuildTargetGroup.PS4))
                {
                ...
                {
                    GUILayout.Label(EditorGUIUtility.TextContent("Streaming"), EditorStyles.boldLabel, new GUILayoutOption[0]);
                    EditorGUILayout.PropertyField(this.m_FirstStreamedSceneWithResources, EditorGUIUtility.TextContent("First Streamed Level"), new GUILayoutOption[0]);
                    EditorGUILayout.Space();
                }
                ...
                {
                    BuildDisabledEnumPopup(EditorGUIUtility.TextContent("Default"), LocalizationDatabase.GetLocalizedString("Scripting Backend"));
                }
                else
                {
                    ScriptingImplementation[] options = scriptingImplementations.Enabled();
                    EditorGUI.BeginChangeCheck();
                    int num3 = BuildEnumPopup<ScriptingImplementation>(LocalizationDatabase.GetLocalizedString("Scripting Backend"), targetGroup, "ScriptingBackend", null, options);
                    if (EditorGUI.EndChangeCheck())
                    {
                        PlayerSettings.SetPropertyInt("ScriptingBackend", num3, targetGroup);
                    }
                    if ((targetGroup == BuildTargetGroup.Android) && (num3 == 1))
                    {
                        EditorGUILayout.HelpBox("IL2CPP on Android is experimental and unsupported", MessageType.Info);
                    }
                }
                if (((targetGroup == BuildTargetGroup.iPhone) || (targetGroup == BuildTargetGroup.Android)) || (((targetGroup == BuildTargetGroup.Metro) || (targetGroup == BuildTargetGroup.BlackBerry)) || IsWP8Player(targetGroup)))
                {
                    if (targetGroup == BuildTargetGroup.iPhone)
                    {
                        ...
                        EditorGUILayout.PropertyField(this.m_TargetResolution, EditorGUIUtility.TextContent("Target Resolution"), new GUILayoutOption[0]);
                    }
                    if (((targetGroup == BuildTargetGroup.iPhone) || (targetGroup == BuildTargetGroup.Metro)) || IsWP8Player(targetGroup))
                    {
                ...
                if ((((targetGroup == BuildTargetGroup.iPhone) || (targetGroup == BuildTargetGroup.XBOX360)) || ((targetGroup == BuildTargetGroup.XboxOne) || (targetGroup == BuildTargetGroup.WiiU))) || (((targetGroup == BuildTargetGroup.PS3) || (targetGroup == BuildTargetGroup.PS4)) || (targetGroup == BuildTargetGroup.PSP2)))
                {
                ...
                if ((((targetGroup == BuildTargetGroup.iPhone) || (targetGroup == BuildTargetGroup.Android)) || ((targetGroup == BuildTargetGroup.BlackBerry) || (targetGroup == BuildTargetGroup.Tizen))) || ((((targetGroup == BuildTargetGroup.WebGL) || (targetGroup == BuildTargetGroup.PS3)) || ((targetGroup == BuildTargetGroup.WiiU) || (targetGroup == BuildTargetGroup.PSP2))) || (((targetGroup == BuildTargetGroup.PS4) || (targetGroup == BuildTargetGroup.XBOX360)) || (targetGroup == BuildTargetGroup.XboxOne))))
                {
                    if (targetGroup == BuildTargetGroup.iPhone)
                    {
                        EditorGUILayout.PropertyField(this.m_IPhoneSdkVersion, EditorGUIUtility.TextContent("SDK Version"), new GUILayoutOption[0]);
                        EditorGUILayout.PropertyField(this.m_IPhoneTargetOSVersion, EditorGUIUtility.TextContent("Target iOS Version"), new GUILayoutOption[0]);
                    }
                    int num4 = -1;
                    PlayerSettings.GetPropertyOptionalInt("ScriptingBackend", ref num4, targetGroup);
                    if ((targetGroup == BuildTargetGroup.WebGL) || (num4 == 1))
                    {
                        EditorGUILayout.PropertyField(this.m_StripEngineCode, EditorGUIUtility.TextContent("Strip Engine Code*|Strip Unused Engine Code"), new GUILayoutOption[0]);
                    }
                    else
                    {
            ...
            if ((((targetGroup == BuildTargetGroup.Metro) || (targetGroup == BuildTargetGroup.XBOX360)) || ((targetGroup == BuildTargetGroup.PS3) || (targetGroup == BuildTargetGroup.PSP2))) || ((targetGroup == BuildTargetGroup.PSM) || ((settingsExtension != null) && settingsExtension.HasPublishSection())))
            {
                ...
                if (this.BeginSettingsBox(5, EditorGUIUtility.TextContent("Publishing Settings")))
                {
                    ...
                    if (targetGroup == BuildTargetGroup.XBOX360)
                    {
                        ...
                        System.Action onSelect = delegate {
        ...
        private void SplashSectionGUI(BuildPlayerWindow.BuildPlatform platform, BuildTargetGroup targetGroup, ISettingEditorExtension settingsExtension)
        {
            ...
            if (this.BeginSettingsBox(2, EditorGUIUtility.TextContent("Splash Image")))
            {
                ...
                bool flag = (targetGroup == BuildTargetGroup.iPhone) || (targetGroup == BuildTargetGroup.Android);
                ...
                bool flag2 = InternalEditorUtility.HasAdvancedLicenseOnBuildTarget(platform.DefaultTarget) || (targetGroup == BuildTargetGroup.iPhone);
                EditorGUI.BeginDisabledGroup(!flag2);
        ...
        [CompilerGenerated]
        private sealed class <GraphicsAPIsGUIOnePlatform>c__AnonStorey8F
        {
        ...
        [CompilerGenerated]
        private sealed class <GraphicsAPIsGUIOnePlatform>c__AnonStorey90
        {
            internal PlayerSettingsEditor.<GraphicsAPIsGUIOnePlatform>c__AnonStorey8F <>f__ref$143;
            internal PlayerSettingsEditor <>f__this;
            internal string displayTitle;

            internal void <>m__1A0(Rect rect, ReorderableList list)
            {
                this.<>f__this.AddGraphicsDeviceElement(this.<>f__ref$143.targetPlatform, rect, list);
            }

            internal void <>m__1A1(ReorderableList list)
            {
                this.<>f__this.RemoveGraphicsDeviceElement(this.<>f__ref$143.targetPlatform, list);
            }

            internal void <>m__1A2(ReorderableList list)
            {
                this.<>f__this.ReorderGraphicsDeviceElement(this.<>f__ref$143.targetPlatform, list);
            }

            internal void <>m__1A3(Rect rect, int index, bool isActive, bool isFocused)
            {
                this.<>f__this.DrawGraphicsDeviceElement(this.<>f__ref$143.targetPlatform, rect, index, isActive, isFocused);
            }

            internal void <>m__1A4(Rect rect)
            {

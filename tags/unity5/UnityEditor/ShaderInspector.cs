namespace UnityEditor
{
    ...
    internal class ShaderInspector : Editor
    {
        ...
        {
            <ShaderErrorListUI>c__AnonStorey89 storey = new <ShaderErrorListUI>c__AnonStorey89 {
                errors = errors
            };
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    <ShaderErrorListUI>c__AnonStorey8A storeya = new <ShaderErrorListUI>c__AnonStorey8A {
                        <>f__ref$137 = storey
                    };
                    current.Use();
                    GenericMenu menu = new GenericMenu();
                    storeya.errorIndex = i;
                    menu.AddItem(new GUIContent("Copy error text"), false, new GenericMenu.MenuFunction(storeya.<>m__186));
                    menu.ShowAsContext();
                }
        ...
        [CompilerGenerated]
        private sealed class <ShaderErrorListUI>c__AnonStorey89
        {
            internal ShaderError[] errors;
        }

        [CompilerGenerated]
        private sealed class <ShaderErrorListUI>c__AnonStorey8A
        {
            internal ShaderInspector.<ShaderErrorListUI>c__AnonStorey89 <>f__ref$137;
            internal int errorIndex;

            internal void <>m__186()
            {
                string message = this.<>f__ref$137.errors[this.errorIndex].message;
                if (!string.IsNullOrEmpty(this.<>f__ref$137.errors[this.errorIndex].messageDetails))
                {
                    message = message + ((string) '\n') + this.<>f__ref$137.errors[this.errorIndex].messageDetails;
                }

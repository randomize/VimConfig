namespace UnityEditor
{
    ...
    internal class AttributeHelper
    {
            ...
            foreach (System.Type type in AssemblyHelper.GetTypesFromAssembly(assembly))
            {
                ...
                for (int i = 0; i < methods.GetLength(0); i++)
                {
                    ...
                    foreach (DrawGizmo gizmo in info.GetCustomAttributes(typeof(DrawGizmo), false))
                    {
                        ...
                        {
                            UnityEngine.Debug.LogWarning(string.Format("Method {0}.{1} is marked with the DrawGizmo attribute but does not take parameters (ComponentType, GizmoType) so will be ignored.", info.DeclaringType.FullName, info.Name));
                        }
                        else
                        {
                            ...
                            {
                                UnityEngine.Debug.LogWarning(string.Format("Method {0}.{1} is marked with the DrawGizmo attribute but the component type it applies to could not be determined.", info.DeclaringType.FullName, info.Name));
                                goto Label_018E;
                            }
                            if ((parameters[1].ParameterType != typeof(GizmoType)) && (parameters[1].ParameterType != typeof(int)))
                            {
                                UnityEngine.Debug.LogWarning(string.Format("Method {0}.{1} is marked with the DrawGizmo attribute but does not take a second parameter of type GizmoType so will be ignored.", info.DeclaringType.FullName, info.Name));
                            }
                            else
                            {
                                item.drawGizmo = info;
        ...
        [CompilerGenerated]
        private sealed class <CallMethodsWithAttribute>c__Iterator3<T> : IDisposable, IEnumerable, IEnumerator, IEnumerable<T>, IEnumerator<T>
        {
            internal T $current;
            internal int $PC;
            internal object[] <$>arguments;
            internal System.Type <$>attributeType;
            internal Assembly[] <$s_284>__0;
            internal int <$s_285>__1;
            internal System.Type[] <$s_286>__3;
            internal int <$s_287>__4;
            internal MethodInfo[] <$s_288>__6;
            internal int <$s_289>__7;
            ...
            public bool MoveNext()
            {
                ...
                {
                    case 0:
                        this.<$s_284>__0 = EditorAssemblies.loadedAssemblies;
                        this.<$s_285>__1 = 0;
                        while (this.<$s_285>__1 < this.<$s_284>__0.Length)
                        {
                            this.<assembly>__2 = this.<$s_284>__0[this.<$s_285>__1];
                            this.<$s_286>__3 = this.<assembly>__2.GetTypes();
                            this.<$s_287>__4 = 0;
                            while (this.<$s_287>__4 < this.<$s_286>__3.Length)
                            {
                                this.<type>__5 = this.<$s_286>__3[this.<$s_287>__4];
                                this.<$s_288>__6 = this.<type>__5.GetMethods(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static);
                                this.<$s_289>__7 = 0;
                                while (this.<$s_289>__7 < this.<$s_288>__6.Length)
                                {
                                    this.<method>__8 = this.<$s_288>__6[this.<$s_289>__7];

namespace UnityEditor
{
    ...
    internal class PropertyHandler
    {
        ...
        {
            <AddMenuItems>c__AnonStorey9A storeya = new <AddMenuItems>c__AnonStorey9A {
                property = property,
                <>f__this = this
            };
            if (this.contextMenuItems != null)
            {
                System.Type type = storeya.property.serializedObject.targetObject.GetType();
                foreach (ContextMenuItemAttribute attribute in this.contextMenuItems)
                {
                    <AddMenuItems>c__AnonStorey9B storeyb = new <AddMenuItems>c__AnonStorey9B {
                        <>f__ref$154 = storeya,
                        <>f__this = this,
                        method = type.GetMethod(attribute.function, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance)
                    };
                    if (storeyb.method != null)
                    {
                        menu.AddItem(new GUIContent(attribute.name), false, new GenericMenu.MenuFunction(storeyb.<>m__1C6));
                    }
                }
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <AddMenuItems>c__AnonStorey9A
        {
        ...
        [CompilerGenerated]
        private sealed class <AddMenuItems>c__AnonStorey9B
        {
            internal PropertyHandler.<AddMenuItems>c__AnonStorey9A <>f__ref$154;
            internal PropertyHandler <>f__this;
            internal MethodInfo method;

            internal void <>m__1C6()
            {
                this.<>f__this.CallMenuCallback(this.<>f__ref$154.property.serializedObject.targetObjects, this.method);
            }
        }
    }
}

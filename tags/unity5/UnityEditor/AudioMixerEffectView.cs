namespace UnityEditor
{
    ...
    internal class AudioMixerEffectView
    {
        ...
        {
            <AudioMixerEffectView>c__AnonStorey59 storey = new <AudioMixerEffectView>c__AnonStorey59();
            ...
            storey.pluginType = typeof(IAudioEffectPluginGUI);
            foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                try
                {
                    IEnumerator<System.Type> enumerator = assembly.GetTypes().Where<System.Type>(new Func<System.Type, bool>(storey.<>m__95)).GetEnumerator();
                    try
                    {
        ...
        {
            <ShowEffectContextMenu>c__AnonStorey5A storeya = new <ShowEffectContextMenu>c__AnonStorey5A {
                effect = effect,
            ...
            GenericMenu pm = new GenericMenu();
            if (!storeya.effect.IsReceive())
            {
                if ((!storeya.effect.IsAttenuation() && !storeya.effect.IsSend()) && !storeya.effect.IsDuckVolume())
                {
                    pm.AddItem(new GUIContent("Allow Wet Mixing (causes higher memory usage)"), storeya.effect.enableWetMix, new GenericMenu.MenuFunction(storeya.<>m__96));
                    pm.AddItem(new GUIContent("Bypass"), storeya.effect.bypass, new GenericMenu.MenuFunction(storeya.<>m__97));
                    pm.AddSeparator(string.Empty);
                }
                pm.AddItem(new GUIContent("Copy effect settings to all snapshots"), false, new GenericMenu.MenuFunction(storeya.<>m__98));
                if ((!storeya.effect.IsAttenuation() && !storeya.effect.IsSend()) && (!storeya.effect.IsDuckVolume() && storeya.effect.enableWetMix))
                {
                    pm.AddItem(new GUIContent("Copy effect settings to all snapshots, including wet level"), false, new GenericMenu.MenuFunction(storeya.<>m__99));
                }
                pm.AddSeparator(string.Empty);
            }
            AudioMixerGroupController[] groups = new AudioMixerGroupController[] { storeya.group };
            AudioMixerChannelStripView.AddEffectItemsToMenu(storeya.controller, groups, storeya.effectIndex, "Add effect before/", pm);
            AudioMixerChannelStripView.AddEffectItemsToMenu(storeya.controller, groups, storeya.effectIndex + 1, "Add effect after/", pm);
            if (!storeya.effect.IsAttenuation())
            {
                pm.AddSeparator(string.Empty);
                pm.AddItem(new GUIContent("Remove this effect"), false, new GenericMenu.MenuFunction(storeya.<>m__9A));
            }
            pm.DropDown(buttonRect);
        }

        [CompilerGenerated]
        private sealed class <AudioMixerEffectView>c__AnonStorey59
        {
            internal System.Type pluginType;

            internal bool <>m__95(System.Type t)
            {
        ...
        [CompilerGenerated]
        private sealed class <ShowEffectContextMenu>c__AnonStorey5A
        {

namespace UnityEditor.Connect
{
    ...
    internal class UnityConnectServiceCollection
    {
        ...
        {
            <GetServiceFromUrl>c__AnonStoreyA1 ya = new <GetServiceFromUrl>c__AnonStoreyA1 {
                searchUrl = searchUrl
            };
            return this.m_Services.FirstOrDefault<KeyValuePair<string, UnityConnectServiceData>>(new Func<KeyValuePair<string, UnityConnectServiceData>, bool>(ya.<>m__1EC)).Value;
        }
        ...
        protected void InstanceStateChanged(ConnectInfo state)
        {
            if (this.isDrawerOpen && state.ready)
            {
                ...
                {
                    bool forceFocus = (s_UnityConnectEditorWindow != null) && s_UnityConnectEditorWindow.webView.HasApplicationFocus();
        ...
        [CompilerGenerated]
        private sealed class <GetServiceFromUrl>c__AnonStoreyA1
        {
            internal string searchUrl;

            internal bool <>m__1EC(KeyValuePair<string, UnityConnectServiceData> kvp)
            {

namespace UnityEditor
{
    ...
    internal static class LODGroupGUI
    {
        ...
        {
            <SetSelectedLODLevelPercentage>c__AnonStorey85 storey = new <SetSelectedLODLevelPercentage>c__AnonStorey85 {
                lods = lods,
                lod = lod
            };
            float screenPercent = 0f;
            LODInfo info = storey.lods.FirstOrDefault<LODInfo>(new Func<LODInfo, bool>(storey.<>m__153));
            if (info != null)
            {
                screenPercent = info.ScreenPercent;
            }
            float num2 = 1f;
            LODInfo info2 = storey.lods.FirstOrDefault<LODInfo>(new Func<LODInfo, bool>(storey.<>m__154));
            if (info2 != null)
            {
            ...
            storey.lods[storey.lod].ScreenPercent = Mathf.Clamp(newScreenPercentage, screenPercent, num2);
        }
        ...
        [CompilerGenerated]
        private sealed class <SetSelectedLODLevelPercentage>c__AnonStorey85
        {
            internal int lod;
            internal List<LODGroupGUI.LODInfo> lods;

            internal bool <>m__153(LODGroupGUI.LODInfo x)
            {
                return (x.LODLevel == (this.lods[this.lod].LODLevel + 1));
            }

            internal bool <>m__154(LODGroupGUI.LODInfo x)
            {
                return (x.LODLevel == (this.lods[this.lod].LODLevel - 1));
            }
        }

namespace UnityEditor.VersionControl
{
    ...
    internal class WindowPending : EditorWindow
    {
        ...
        {
            <GetEmptyChangeSetsCandidates>c__AnonStoreyAA yaa = new <GetEmptyChangeSetsCandidates>c__AnonStoreyAA();
            ChangeSets emptyChangeSets = this.pendingList.EmptyChangeSets;
            yaa.toDelete = new ChangeSets();
            if (<>f__am$cacheD == null)
            {
                <>f__am$cacheD = item => item.id != ChangeSet.defaultID;
            }
            emptyChangeSets.FindAll(<>f__am$cacheD).ForEach(new Action<ChangeSet>(yaa.<>m__1F8));
            return yaa.toDelete;
        }
        ...
        private void PopulateListControl(ListControl list, Task task, Texture2D icon)
        {
            ...
            {
                <PopulateListControl>c__AnonStoreyA9 ya = new <PopulateListControl>c__AnonStoreyA9 {
                    cs = firstChild.Item as ChangeSet
                };
                if (changeSets.Find(new Predicate<ChangeSet>(ya.<>m__1F6)) == null)
                {
        ...
        [CompilerGenerated]
        private sealed class <GetEmptyChangeSetsCandidates>c__AnonStoreyAA
        {
            internal ChangeSets toDelete;

            internal void <>m__1F8(ChangeSet s)
            {
                this.toDelete.Add(s);
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <PopulateListControl>c__AnonStoreyA9
        {
            internal ChangeSet cs;

            internal bool <>m__1F6(ChangeSet elm)
            {

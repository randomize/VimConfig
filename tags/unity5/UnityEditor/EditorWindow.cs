namespace UnityEditor
{
    ...
    public class EditorWindow : ScriptableObject
    {
        ...
        {
            UnityEngine.Object[] objArray = Resources.FindObjectsOfTypeAll(t);
        ...
        public static T GetWindow<T>(string title, bool focus, params System.Type[] desiredDockNextTo) where T: EditorWindow
        {
            ...
            <GetWindow>c__AnonStoreyB<T> yb = new <GetWindow>c__AnonStoreyB<T>();
            System.Type[] typeArray = desiredDockNextTo;
            for (int i = 0; i < typeArray.Length; i++)
            {
                yb.desired = typeArray[i];
                foreach (ContainerWindow window in ContainerWindow.windows)
                {
                    foreach (View view in window.mainView.allChildren)
                    {
                        ...
                        if ((area != null) && area.m_Panes.Any<EditorWindow>(new Func<EditorWindow, bool>(yb.<>m__9)))
                        {
                            area.AddTab(pane);
                            return pane;
                        }
        ...
        {
            UnityEngine.Object[] objArray = Resources.FindObjectsOfTypeAll(typeof(T));
        ...
        [CompilerGenerated]
        private sealed class <GetWindow>c__AnonStoreyB<T> where T: EditorWindow
        {
            internal System.Type desired;

            internal bool <>m__9(EditorWindow pane)
            {

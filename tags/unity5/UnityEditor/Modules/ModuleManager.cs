namespace UnityEditor.Modules
{
    ...
    internal static class ModuleManager
    {
        ...
        [CompilerGenerated]
        private static Func<Assembly, System.Type> <>f__am$cache8;
        [CompilerGenerated]
        ...
        {
            EditorUserBuildSettings.activeBuildTargetChanged = (System.Action) Delegate.Combine(EditorUserBuildSettings.activeBuildTargetChanged, new System.Action(ModuleManager.OnActiveBuildTargetChanged));
        }
        ...
        private static bool InitializePackageManager(Assembly assembly, Unity.DataContract.PackageInfo package)
        {
            ...
            {
                Unity.DataContract.PackageInfo info2 = new Unity.DataContract.PackageInfo {
            ...
            IEnumerator<Unity.DataContract.PackageInfo> enumerator = s_PackageManager.playbackEngines.GetEnumerator();
            try
            {
                while (enumerator.MoveNext())
                {
        ...
        {
            IEnumerator<Unity.DataContract.PackageInfo> enumerator = s_PackageManager.unityExtensions.GetEnumerator();
            try
            {
                while (enumerator.MoveNext())
                {
        ...
        {
            <RegisterModulesFromLoadedAssemblies>c__AnonStorey94<T> storey = new <RegisterModulesFromLoadedAssemblies>c__AnonStorey94<T> {
            ...
            return AppDomain.CurrentDomain.GetAssemblies().Aggregate<Assembly, List<T>>(new List<T>(), new Func<List<T>, Assembly, List<T>>(storey.<>m__1B5));
        }
        ...
        private static void RegisterPackageManager()
        {
            ...
            System.Type type = AppDomain.CurrentDomain.GetAssemblies().Where<Assembly>(<>f__am$cache7).Select<Assembly, System.Type>(<>f__am$cache8).FirstOrDefault<System.Type>();
            try
            {
        ...
        [CompilerGenerated]
        private sealed class <RegisterModulesFromLoadedAssemblies>c__AnonStorey94<T>
        {
            internal Func<Assembly, IEnumerable<T>> processAssembly;

            internal List<T> <>m__1B5(List<T> list, Assembly assembly)
            {
                try
                {

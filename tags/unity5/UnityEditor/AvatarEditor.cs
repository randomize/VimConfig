namespace UnityEditor
{
    ...
    internal class AvatarEditor : Editor
    {
        ...
        internal void SwitchToAssetMode()
        {
            ...
            {
                <SwitchToAssetMode>c__AnonStorey8B storeyb;
                storeyb = new <SwitchToAssetMode>c__AnonStorey8B {
                    CleanUpSceneOnDestroy = null,
                    userFileName = this.m_UserFileName,
                    CleanUpSceneOnDestroy = new EditorApplication.CallbackFunction(storeyb.<>m__19C)
                };
                EditorApplication.update = (EditorApplication.CallbackFunction) Delegate.Combine(EditorApplication.update, storeyb.CleanUpSceneOnDestroy);
            }
        ...
        [CompilerGenerated]
        private sealed class <SwitchToAssetMode>c__AnonStorey8B
        {
            ...
            internal void <>m__19C()
            {
                if (EditorApplication.currentScene.Length <= 0)
                {
                    if (this.userFileName.Length > 0)
                    {
                        EditorApplication.OpenScene(this.userFileName);
                    }
                    else
                    {
                        EditorApplication.NewScene();

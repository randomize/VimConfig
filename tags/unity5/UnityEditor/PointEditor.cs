namespace UnityEditor
{
    ...
    internal class PointEditor
    {
        ...
        {
            <MovePoints>c__AnonStorey36 storey = new <MovePoints>c__AnonStorey36 {
                points = points
            };
            if (selection.Count != 0)
            {
                ...
                {
                    Vector3 zero = Vector3.zero;
                    zero = (Tools.pivotMode != PivotMode.Pivot) ? ((Vector3) (selection.Aggregate<int, Vector3>(zero, new Func<Vector3, int, Vector3>(storey.<>m__45)) / ((float) selection.Count))) : storey.points.GetPosition(selection[0]);
                    ...
                    switch (Tools.current)
                    {
                        ...
                        case Tool.Scale:
                        {
                            ...
                            if (GUI.changed)
                            {
                                ...
                                {
                                    Vector3 vector8 = storey.points.GetPosition(num3) - vector7;
                                    ...
                                    vector8.z *= vector6.z;
                                    vector8 += vector7;
                                    storey.points.SetPosition(num3, vector8);
                                }
        ...
        [CompilerGenerated]
        private sealed class <MovePoints>c__AnonStorey36
        {
            internal IEditablePoint points;

            internal Vector3 <>m__45(Vector3 current, int index)
            {

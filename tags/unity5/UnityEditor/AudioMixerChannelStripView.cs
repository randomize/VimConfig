namespace UnityEditor
{
    ...
    internal class AudioMixerChannelStripView
    {
        ...
        private void DoEffectSlotInsertEffectPopup(Rect buttonRect, AudioMixerGroupController group, List<AudioMixerGroupController> allGroups, int effectSlotIndex, ref Dictionary<AudioMixerEffectController, AudioMixerGroupController> effectMap)
        {
            ...
            {
                <DoEffectSlotInsertEffectPopup>c__AnonStorey58 storey = new <DoEffectSlotInsertEffectPopup>c__AnonStorey58 {
                    <>f__this = this,
                    effect = group.effects[effectSlotIndex]
                };
                if ((!storey.effect.IsAttenuation() && !storey.effect.IsSend()) && (!storey.effect.IsReceive() && !storey.effect.IsDuckVolume()))
                {
                    pm.AddItem(new GUIContent("Allow Wet Mixing (causes higher memory usage)"), storey.effect.enableWetMix, new GenericMenu.MenuFunction(storey.<>m__91));
                    pm.AddItem(new GUIContent("Bypass"), storey.effect.bypass, new GenericMenu.MenuFunction(storey.<>m__92));
                    pm.AddSeparator(string.Empty);
                }
        ...
        {
            <GroupClicked>c__AnonStorey57 storey = new <GroupClicked>c__AnonStorey57();
            ...
            Selection.objects = p.allGroups.Where<AudioMixerGroupController>(new Func<AudioMixerGroupController, bool>(storey.<>m__90)).ToList<AudioMixerGroupController>().ToArray();
        ...
        [CompilerGenerated]
        private sealed class <DoEffectSlotInsertEffectPopup>c__AnonStorey58
        {
        ...
        [CompilerGenerated]
        private sealed class <GroupClicked>c__AnonStorey57
        {
            internal List<int> newSelection;

            internal bool <>m__90(AudioMixerGroupController x)
            {

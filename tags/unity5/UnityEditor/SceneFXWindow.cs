namespace UnityEditor
{
    ...
    internal class SceneFXWindow : PopupWindowContent
    {
        ...
        {
            <Draw>c__AnonStorey43 storey = new <Draw>c__AnonStorey43();
            if ((this.m_SceneView != null) && (this.m_SceneView.m_SceneViewState != null))
            {
                ...
                this.DrawListElement(rect2, "Skybox", storey.state.showSkybox, new Action<bool>(storey.<>m__72));
                rect2.y += 16f;
                this.DrawListElement(rect2, "Fog", storey.state.showFog, new Action<bool>(storey.<>m__73));
                rect2.y += 16f;
                this.DrawListElement(rect2, "Flares", storey.state.showFlares, new Action<bool>(storey.<>m__74));
                rect2.y += 16f;
                this.DrawListElement(rect2, "Animated Materials", storey.state.showMaterialUpdate, new Action<bool>(storey.<>m__75));
                rect2.y += 16f;
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <Draw>c__AnonStorey43
        {
            internal SceneView.SceneViewState state;

            internal void <>m__72(bool value)
            {
                this.state.showSkybox = value;
            }

            internal void <>m__73(bool value)
            {
                this.state.showFog = value;
            }

            internal void <>m__74(bool value)
            {
                this.state.showFlares = value;
            }

            internal void <>m__75(bool value)
            {

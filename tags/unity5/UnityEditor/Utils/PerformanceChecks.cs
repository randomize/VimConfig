namespace UnityEditor.Utils
{
    ...
    internal class PerformanceChecks
    {
        ...
        {
            <CheckMaterial>c__AnonStoreyA6 ya = new <CheckMaterial>c__AnonStoreyA6();
            if ((mat != null) && (mat.shader != null))
            {
                ya.shaderName = mat.shader.name;
                int lOD = ShaderUtil.GetLOD(mat.shader);
                bool flag = Array.Exists<string>(kShadersWithMobileVariants, new Predicate<string>(ya.<>m__1F3));
                ...
                if (mat.GetTag("PerformanceChecks", true).ToLower() != "false")
                {
                    if (flag)
                    {
                        ...
                        {
                            object[] args = new object[] { "Mobile/" + ya.shaderName };
                            return FormattedTextContent("Shader is using white color which does nothing; Consider using {0} shader for performance.", args);
                        }
                        if (flag2 && ya.shaderName.StartsWith("Particles/"))
                        {
                            object[] objArray2 = new object[] { "Mobile/" + ya.shaderName };
                            return FormattedTextContent("Consider using {0} shader on this platform for performance.", objArray2);
                        }
                        if (((ya.shaderName == "RenderFX/Skybox") && mat.HasProperty("_Tint")) && (mat.GetColor("_Tint") == new Color(0.5f, 0.5f, 0.5f, 0.5f)))
                        {
                    ...
                    if (((lOD >= 300) && flag2) && !ya.shaderName.StartsWith("Mobile/"))
                    {
                    ...
                    if (ya.shaderName.Contains("VertexLit") && mat.HasProperty("_Emission"))
                    {
        ...
        [CompilerGenerated]
        private sealed class <CheckMaterial>c__AnonStoreyA6
        {
            internal string shaderName;

            internal bool <>m__1F3(string s)
            {

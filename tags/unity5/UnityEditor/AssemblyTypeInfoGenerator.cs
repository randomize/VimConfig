namespace UnityEditor
{
    ...
    internal class AssemblyTypeInfoGenerator
    {
        ...
        {
            <AddType>c__AnonStorey5C storeyc = new <AddType>c__AnonStorey5C {
                typeRef = typeRef,
                <>f__this = this
            };
            if (!this.classes_.Any<ClassInfo>(new Func<ClassInfo, bool>(storeyc.<>m__BC)))
            {
                TypeDefinition definition;
                try
                {
                    definition = storeyc.typeRef.Resolve();
                }
                ...
                if (definition != null)
                {
                    if (storeyc.typeRef.IsGenericInstance)
                    {
                        Collection<TypeReference> genericArguments = ((GenericInstanceType) storeyc.typeRef).GenericArguments;
                        ...
                        this.typeResolver.Add((GenericInstanceType) storeyc.typeRef);
                    }
                    bool flag = false;
                    try
                    {
                    ...
                    else
                    {
                        ClassInfo item = new ClassInfo {
                            name = this.GetMonoEmbeddedFullTypeNameFor(storeyc.typeRef),
                            fields = this.GetFields(definition, storeyc.typeRef.IsGenericInstance, genericInstanceTypeMap)
                        };
                        this.classes_.Add(item);
                        this.AddNestedTypes(definition, genericInstanceTypeMap);
                        this.AddBaseType(storeyc.typeRef, genericInstanceTypeMap);
                    }
                    if (storeyc.typeRef.IsGenericInstance)
                    {
                        this.typeResolver.Remove((GenericInstanceType) storeyc.typeRef);
                    }
                }
            }
        }
        ...
        {
            <MakeGenericInstance>c__AnonStorey5D storeyd = new <MakeGenericInstance>c__AnonStorey5D {
                genericInstanceTypeMap = genericInstanceTypeMap,
                <>f__this = this
            };
            GenericInstanceType type = new GenericInstanceType(genericClass);
            IEnumerator<TypeReference> enumerator = arguments.Select<TypeReference, TypeReference>(new Func<TypeReference, TypeReference>(storeyd.<>m__BD)).GetEnumerator();
            try
            {
        ...
        [CompilerGenerated]
        private sealed class <AddType>c__AnonStorey5C
        {
            internal AssemblyTypeInfoGenerator <>f__this;
            internal TypeReference typeRef;

            internal bool <>m__BC(AssemblyTypeInfoGenerator.ClassInfo x)
            {
        ...
        [CompilerGenerated]
        private sealed class <MakeGenericInstance>c__AnonStorey5D
        {
            internal AssemblyTypeInfoGenerator <>f__this;
            internal Dictionary<TypeReference, TypeReference> genericInstanceTypeMap;

            internal TypeReference <>m__BD(TypeReference x)
            {

    ...
    internal class WheelJoint2DEditor : AnchoredJoint2DEditor
    {
        public void OnSceneGUI()
        {
            ...
            if (target.enabled)
            {
                ...
                Vector3 vector4 = (Vector3) Joint2DEditorBase.RotateVector2(Vector3.right, -target.suspension.angle - target.transform.eulerAngles.z);
                Handles.color = Color.green;
                vector4 = (Vector3) (vector4 * (HandleUtility.GetHandleSize(position) * 0.3f));
                start += vector4;
                end -= vector4;
                Joint2DEditorBase.DrawAALine(start, end);
                base.OnSceneGUI();
            }
        }
    }

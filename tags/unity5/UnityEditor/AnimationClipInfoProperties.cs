    ...
    internal class AnimationClipInfoProperties
    {
        ...
        public bool MaskNeedsUpdating()
        {
            ...
            if (maskSource != null)
            {
                ...
                {
                    if (maskSource.GetHumanoidBodyPartActive(i) != (property.GetArrayElementAtIndex(i).intValue != 0))
                    {
                        return true;
                    }
                }
                ...
                for (int j = 0; j < arraySize; j++)
                {
                    SerializedProperty property3 = property2.GetArrayElementAtIndex(j).FindPropertyRelative("m_Path");
                    SerializedProperty property4 = property2.GetArrayElementAtIndex(j).FindPropertyRelative("m_Weight");
                    if ((maskSource.GetTransformPath(j) != property3.stringValue) || (maskSource.GetTransformActive(j) != (property4.floatValue > 0.5f)))
                    {
                        return true;
                    }
                }

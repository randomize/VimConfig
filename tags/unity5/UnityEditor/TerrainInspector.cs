namespace UnityEditor
{
    ...
    internal class TerrainInspector : Editor
    {
        ...
        public void ShowResolution()
        {
            ...
            x = EditorGUI.DelayedFloatField(EditorGUIUtility.TempContent("Terrain Width"), x);
            if (x <= 0f)
            {
                x = 1f;
            }
            ...
            z = EditorGUI.DelayedFloatField(EditorGUIUtility.TempContent("Terrain Length"), z);
            if (z <= 0f)
            {
                z = 1f;
            }
            ...
            y = EditorGUI.DelayedFloatField(EditorGUIUtility.TempContent("Terrain Height"), y);
            if (y <= 0f)
            {
                y = 1f;
            }
            ...
            heightmapResolution = Mathf.Clamp(EditorGUI.DelayedIntField(EditorGUIUtility.TempContent("Heightmap Resolution"), heightmapResolution), 0x21, 0x1001);
            heightmapResolution = this.m_Terrain.terrainData.GetAdjustedSize(heightmapResolution);
            detailResolution = Mathf.Clamp(EditorGUI.DelayedIntField(EditorGUIUtility.TempContent("Detail Resolution"), detailResolution), 0, 0xfd0);
            detailResolutionPerPatch = Mathf.Clamp(EditorGUI.DelayedIntField(EditorGUIUtility.TempContent("Detail Resolution Per Patch"), detailResolutionPerPatch), 8, 0x80);
            alphamapResolution = Mathf.Clamp(Mathf.ClosestPowerOfTwo(EditorGUI.DelayedIntField(EditorGUIUtility.TempContent("Control Texture Resolution"), alphamapResolution)), 0x10, 0x800);
            baseMapResolution = Mathf.Clamp(Mathf.ClosestPowerOfTwo(EditorGUI.DelayedIntField(EditorGUIUtility.TempContent("Base Texture Resolution"), baseMapResolution)), 0x10, 0x800);
        ...
        public void ShowSettings()
        {
            ...
            float num = EditorGUILayout.Slider("Speed", terrainData.wavingGrassStrength, 0f, 1f, new GUILayoutOption[0]);
            float num2 = EditorGUILayout.Slider("Size", terrainData.wavingGrassSpeed, 0f, 1f, new GUILayoutOption[0]);
            float num3 = EditorGUILayout.Slider("Bending", terrainData.wavingGrassAmount, 0f, 1f, new GUILayoutOption[0]);
            Color color = EditorGUILayout.ColorField("Grass Tint", terrainData.wavingGrassTint, new GUILayoutOption[0]);
            if (EditorGUI.EndChangeCheck())
            {
                terrainData.wavingGrassStrength = num;

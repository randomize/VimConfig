namespace UnityEditor
{
    ...
    internal class PluginImporterInspector : AssetImporterInspector
    {
        ...
        internal override void ResetValues()
        {
            ...
            <ResetValues>c__AnonStorey72 storey = new <ResetValues>c__AnonStorey72();
            using (List<BuildTarget>.Enumerator enumerator = GetValidBuildTargets().GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    storey.platform = enumerator.Current;
                    this.ResetCompatability(ref this.m_CompatibleWithPlatform[(int) storey.platform], new GetComptability(storey.<>m__105));
                }
            }
            if (this.IsEditingPlatformSettingsSupported())
            {
        ...
        [CompilerGenerated]
        private sealed class <ResetValues>c__AnonStorey72
        {
            internal BuildTarget platform;

            internal bool <>m__105(PluginImporter imp)
            {

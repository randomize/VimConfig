    ...
    internal class ProjectBrowser : EditorWindow, IHasCustomMenu
    {
        ...
        public virtual void AddItemsToMenu(GenericMenu menu)
        {
            if (this.m_EnableOldAssetTree)
            {
                ...
                {
                    menu.AddItem(new GUIContent("Debug TreeView"), false, new GenericMenu.MenuFunction(this.OpenTreeViewTestWindow));
                }
            }
        }
        ...
        public static bool IsFolder(int instanceID)
        {
            ...
            IHierarchyProperty property = new HierarchyProperty(HierarchyType.Assets);
            return (property.Find(instanceID, null) && property.isFolder);
        }
        ...
        private void OnSelectionChange()
        {
            if (this.m_ListArea != null)
            {
                ...
                else if ((this.m_ViewMode == ViewMode.TwoColumns) && !this.m_InternalSelectionChange)
                {
                    bool flag2 = !this.m_IsLocked && (Selection.instanceIDs.Length > 0);
                    if (flag2)
                    {
                        ...
                        {
                            this.FrameObjectPrivate(instanceID, true, false);
                        }
                    }
                }
        ...
        private void SelectAll()
        {
            if (this.m_ViewMode == ViewMode.OneColumn)
            {
                ...
                {
                    int[] visibleRowIDs = this.m_AssetTree.GetVisibleRowIDs();
                    this.m_AssetTree.SetSelection(visibleRowIDs, false);
                    this.AssetTreeSelectionCallback(visibleRowIDs);
                }
            }
            else if (this.m_ViewMode == ViewMode.TwoColumns)
            {
        ...
        private void SetSearchFoldersFromCurrentSelection()
        {
            ...
            {
                HierarchyProperty property = new HierarchyProperty(HierarchyType.Assets);
                if (property.Find(num, null))
                {
                    int num3 = 0;
                    while ((property.isValid && !property.isFolder) && (num3++ < 3))
                    {
                        property.Parent();
                    }
                    if (property.isValid && property.isFolder)
                    {
                        source.Add(AssetDatabase.GetAssetPath(property.instanceID));
                    }
                }
            }
        ...
        {
            if ((((this.m_ViewMode == ViewMode.TwoColumns) && (this.m_SearchFilter.GetState() == SearchFilter.State.FolderBrowsing)) && ((this.m_SearchFilter.folders.Length > 0) && !pathName.StartsWith("assets/", StringComparison.CurrentCultureIgnoreCase))) && (Selection.GetFiltered(typeof(UnityEngine.Object), UnityEditor.SelectionMode.Assets).Length == 0))
            {

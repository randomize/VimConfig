    ...
    internal class SliderJoint2DEditor : AnchoredJoint2DEditor
    {
        public void OnSceneGUI()
        {
            ...
            if (target.enabled)
            {
                ...
                Vector3 lhs = (Vector3) Joint2DEditorBase.RotateVector2(Vector3.right, -target.angle - target.transform.eulerAngles.z);
                ...
                if (target.useLimits)
                {
                    ...
                    Joint2DEditorBase.DrawAALine(vector2 + ((Vector3) (vector5 * num)), vector2 - ((Vector3) (vector5 * num)));
                    Joint2DEditorBase.DrawAALine(vector3 + ((Vector3) (vector5 * num2)), vector3 - ((Vector3) (vector5 * num2)));
                }
                else
                {

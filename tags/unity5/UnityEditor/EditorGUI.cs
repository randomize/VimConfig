namespace UnityEditor
{
    ...
    public sealed class EditorGUI
    {
        ...
        internal static readonly GUIContent[] s_NearAndFarLabels = new GUIContent[] { new GUIContent("Near"), new GUIContent("Far") };
        ...
        {
            return DelayedFloatField(EditorGUILayout.GetControlRect(new GUILayoutOption[0]), label, value, EditorStyles.numberField);
        }
        ...
        internal static float DelayedFloatField(Rect position, GUIContent label, float value, GUIStyle style)
        {
            ...
            BeginChangeCheck();
            string s = DelayedTextField(position, num.ToString(), s_AllowedCharactersForFloat, style);
        ...
        internal static int DelayedIntField(GUIContent label, int value)
        {
            int num = value;
            int result = num;
            Rect position = PrefixLabel(EditorGUILayout.GetControlRect(new GUILayoutOption[0]), label);
            BeginChangeCheck();
            string s = DelayedTextField(position, num.ToString(), s_AllowedCharactersForInt, EditorStyles.numberField);
        ...
        internal static Color DoColorBrightnessField(Rect rect, Rect dragRect, Color col, float minBrightness, float maxBrightness, GUIStyle style)
        {
            ...
            switch (current.GetTypeForControl(controlID))
            {
                case EventType.MouseDown:
                    if (((current.button == 0) && dragRect.Contains(Event.current.mousePosition)) && (GUIUtility.hotControl == 0))
                    {
                        ...
                        if (stateObject != null)
                        {
                            EditorGUIUtility.RGBToHSV(col, out stateObject.m_Hue, out stateObject.m_Saturation, out stateObject.m_Brightness);
                        }
                ...
                case EventType.MouseDrag:
                    if (GUIUtility.hotControl == controlID)
                    {
                        ...
                        col = EditorGUIUtility.HSVToRGB(obj3.m_Hue, obj3.m_Saturation, obj3.m_Brightness, maxBrightness > 1f);
                        GUIUtility.keyboardControl = 0;
                        GUI.changed = true;
                        current.Use();
                    }
            ...
            BeginChangeCheck();
            float num7 = DelayedFloatField(rect, null, col.maxColorComponent, style);
            if (EndChangeCheck())
            {
                float num8;
                float num9;
                float num10;
                EditorGUIUtility.RGBToHSV(col, out num8, out num9, out num10);
                float v = Mathf.Clamp(num7, minBrightness, maxBrightness);
                col = EditorGUIUtility.HSVToRGB(num8, num9, v, maxBrightness > 1f);
            }
            return col;
        }
        ...
        internal static Color DoHexColorTextField(Rect rect, Color color, bool showAlpha, GUIStyle style)
        {
            ...
            {
                Color color2;
                s_RecycledEditor.content.text = s_RecycledEditor.content.text.ToUpper();
        ...
        internal static bool DoObjectFoldout(int id, Rect position, UnityEngine.Object[] targetObjs, bool foldout)
        {
            ...
            switch (typeForControl)
            {
                ...
                default:
                    if ((typeForControl == EventType.ContextClick) && (position.Contains(current.mousePosition) && (targetObjs[0] != null)))
                    {
                        EditorUtility.DisplayObjectContextMenu(new Rect(current.mousePosition.x, current.mousePosition.y, 0f, 0f), targetObjs, 0);
                        current.Use();
                    }
        ...
        internal static string DoTextFieldDropDown(Rect rect, int id, string text, string[] dropDownElements, bool delayed)
        {
            ...
            if (delayed)
            {
                text = DelayedTextField(position, text, null, EditorStyles.textFieldDropDownText);
            }
            else
            {
                bool flag;
        ...
        public static bool InspectorTitlebar(Rect position, bool foldout, UnityEngine.Object[] targetObjs)
        {
            ...
            return DoObjectFoldout(id, position, targetObjs, foldout);
        }
        ...
        internal static string SearchField(Rect position, string text)
        {
            ...
            {
                s_RecycledEditor.content.text = text = string.Empty;
                GUIUtility.keyboardControl = 0;
            }
            return text;
        }
        ...
        internal struct KnobContext
        {
            ...
            private float DoKeyboardInput()
            {
                ...
                string str = EditorGUI.DelayedTextField(position, this.currentValue.ToString("0.##"), null, none);

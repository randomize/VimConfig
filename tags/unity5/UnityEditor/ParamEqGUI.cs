namespace UnityEditor
{
    ...
    internal class ParamEqGUI : IAudioEffectPluginGUI
    {
        ...
        private static bool ParamEqualizerCurveEditor(IAudioEffectPlugin plugin, Rect r, ref float centerFreq, ref float bandwidth, ref float gain, float blend)
        {
            ...
            float num9;
            float num10;
            <ParamEqualizerCurveEditor>c__AnonStorey55 storey = new <ParamEqualizerCurveEditor>c__AnonStorey55 {
                plugin = plugin
            };
            Event current = Event.current;
            int controlID = GUIUtility.GetControlID(FocusType.Passive);
            r = AudioCurveRendering.BeginCurveFrame(r);
            storey.plugin.GetFloatParameterInfo(kCenterFreqName, out num2, out num3, out num4);
            storey.plugin.GetFloatParameterInfo(kOctaveRangeName, out num5, out num6, out num7);
            storey.plugin.GetFloatParameterInfo(kFrequencyGainName, out num8, out num9, out num10);
            bool flag = false;
            switch (current.GetTypeForControl(controlID))
            {
                ...
                case EventType.MouseDrag:
                {
                    ...
                    centerFreq = Mathf.Clamp((float) MapNormalizedFrequency(MapNormalizedFrequency((double) centerFreq, (double) storey.plugin.GetSampleRate(), true, false) + (current.delta.x / r.width), (double) storey.plugin.GetSampleRate(), true, true), num2, num3);
        ...
        Label_01E2:
            ...
            {
                <ParamEqualizerCurveEditor>c__AnonStorey56 storey2 = new <ParamEqualizerCurveEditor>c__AnonStorey56 {
                    <>f__ref$85 = storey
                };
                float num12 = (float) MapNormalizedFrequency((double) centerFreq, (double) storey.plugin.GetSampleRate(), true, false);
                ...
                storey2.wm = (-2.0 * num13) / ((double) storey.plugin.GetSampleRate());
                double a = ((2.0 * num13) * ((double) centerFreq)) / ((double) storey.plugin.GetSampleRate());
                double num15 = 1.0 / ((double) bandwidth);
                double num16 = (double) gain;
                double num17 = Math.Sin(a) / (2.0 * num15);
                storey2.b0 = 1.0 + (num17 * num16);
                storey2.b1 = -2.0 * Math.Cos(a);
                storey2.b2 = 1.0 - (num17 * num16);
                storey2.a0 = 1.0 + (num17 / num16);
                storey2.a1 = -2.0 * Math.Cos(a);
                storey2.a2 = 1.0 - (num17 / num16);
                AudioCurveRendering.DrawCurve(r, new AudioCurveRendering.AudioCurveEvaluator(storey2.<>m__8B), ScaleAlpha(AudioCurveRendering.kAudioOrange, blend));
            }
            DrawFrequencyTickMarks(r, (float) storey.plugin.GetSampleRate(), true, new Color(1f, 1f, 1f, 0.3f * blend));
        ...
        [CompilerGenerated]
        private sealed class <ParamEqualizerCurveEditor>c__AnonStorey55
        {
            internal IAudioEffectPlugin plugin;
        }

        [CompilerGenerated]
        private sealed class <ParamEqualizerCurveEditor>c__AnonStorey56
        {
            internal ParamEqGUI.<ParamEqualizerCurveEditor>c__AnonStorey55 <>f__ref$85;
            internal double a0;
            internal double a1;
            internal double a2;
            internal double b0;
            ...
            internal float <>m__8B(float x)
            {
                double num = ParamEqGUI.MapNormalizedFrequency((double) x, (double) this.<>f__ref$85.plugin.GetSampleRate(), true, true);

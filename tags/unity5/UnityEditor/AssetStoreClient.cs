namespace UnityEditor
{
    ...
    internal class AssetStoreClient
    {
        ...
        static AssetStoreClient()
        {
            ...
            {
                <>f__am$cache4 = new RemoteCertificateValidationCallback(AssetStoreClient.<AssetStoreClient>m__7A);
            }
            ServicePointManager.ServerCertificateValidationCallback = <>f__am$cache4;
        }

        [CompilerGenerated]
        private static bool <AssetStoreClient>m__7A(object, X509Certificate, X509Chain, SslPolicyErrors)
        {
            return true;
        }
        ...
        {
            <AssetsInfo>c__AnonStorey4C storeyc = new <AssetsInfo>c__AnonStorey4C();
            string url = APIUrl("/assets/list");
            foreach (AssetStoreAsset asset in assets)
            {
                url = url + "&id=" + asset.id.ToString();
            }
            storeyc.r = new AssetStoreAssetsInfo(callback, assets);
            return CreateJSONRequest(url, new DoneCallback(storeyc.<>m__7F));
        }
        ...
        {
            <BuildPackage>c__AnonStorey4E storeye = new <BuildPackage>c__AnonStorey4E();
            string url = APIUrl("/content/download/" + asset.packageID.ToString());
            storeye.r = new BuildPackageResult(asset, callback);
            return CreateJSONRequest(url, new DoneCallback(storeye.<>m__81));
        }
        ...
        {
            <DirectPurchase>c__AnonStorey4D storeyd = new <DirectPurchase>c__AnonStorey4D();
            string url = APIUrl(string.Format("/purchase/direct/{0}", packageID.ToString()));
            storeyd.r = new PurchaseResult(callback);
            Dictionary<string, string> param = new Dictionary<string, string>();
            param["password"] = password;
            return CreateJSONRequestPost(url, param, new DoneCallback(storeyd.<>m__80));
        }
        ...
        {
            <SearchAssets>c__AnonStorey4B storeyb = new <SearchAssets>c__AnonStorey4B();
            ...
            storeyb.r = new AssetStoreSearchResults(callback);
            return CreateJSONRequest(url, new DoneCallback(storeyb.<>m__7E));
        }

        private static AsyncHTTPClient.DoneCallback WrapJsonCallback(DoneCallback callback)
        {
            <WrapJsonCallback>c__AnonStorey4A storeya = new <WrapJsonCallback>c__AnonStorey4A {
                callback = callback
            };
            return new AsyncHTTPClient.DoneCallback(storeya.<>m__7C);
        }

        private static AsyncHTTPClient.DoneCallback WrapLoginCallback(DoneLoginCallback callback)
        {
            <WrapLoginCallback>c__AnonStorey49 storey = new <WrapLoginCallback>c__AnonStorey49 {
                callback = callback
            };
            return new AsyncHTTPClient.DoneCallback(storey.<>m__7B);
        }
        ...
        [CompilerGenerated]
        private sealed class <AssetsInfo>c__AnonStorey4C
        {
            internal AssetStoreAssetsInfo r;

            internal void <>m__7F(AssetStoreResponse ar)
            {
                this.r.Parse(ar);
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <BuildPackage>c__AnonStorey4E
        {
            internal BuildPackageResult r;

            internal void <>m__81(AssetStoreResponse ar)
            {
                this.r.Parse(ar);
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <DirectPurchase>c__AnonStorey4D
        {
            internal PurchaseResult r;

            internal void <>m__80(AssetStoreResponse ar)
            {
                this.r.Parse(ar);
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <SearchAssets>c__AnonStorey4B
        {
            internal AssetStoreSearchResults r;

            internal void <>m__7E(AssetStoreResponse ar)
            {
                this.r.Parse(ar);
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <WrapJsonCallback>c__AnonStorey4A
        {
            internal AssetStoreClient.DoneCallback callback;

            internal void <>m__7C(AsyncHTTPClient job)
            {
                if (job.IsDone())
                {
                    try
        ...
        [CompilerGenerated]
        private sealed class <WrapLoginCallback>c__AnonStorey49
        {
            internal AssetStoreClient.DoneLoginCallback callback;

            internal void <>m__7B(AsyncHTTPClient job)
            {
                string text = job.text;
                if (!job.IsSuccess())
                {

namespace UnityEditor
{
    ...
    public sealed class Handles
    {
        ...
        public static void DrawDottedLine(Vector3 p1, Vector3 p2, float screenSpaceSize)
        {
            ...
            {
                Color c = s_Color * new Color(1f, 1f, 1f, 0.75f);
                HandleUtility.ApplyWireMaterial();
                GL.PushMatrix();
                GL.MultMatrix(matrix);
                GL.Begin(1);
                GL.Color(c);
                Vector3 a = current.WorldToScreenPoint(p1);
                Vector3 b = current.WorldToScreenPoint(p2);
                float num = Vector2.Distance(a, b);
                int num2 = Mathf.Min(Mathf.CeilToInt(num / screenSpaceSize), 0x3e8);
                screenSpaceSize = num / ((float) num2);
                for (int i = 0; i < num2; i += 2)
                {
                    GL.Vertex(current.ScreenToWorldPoint(Vector3.Lerp(a, b, (i * screenSpaceSize) / num)));
                    GL.Vertex(current.ScreenToWorldPoint(Vector3.Lerp(a, b, ((i + 1) * screenSpaceSize) / num)));
                }
                GL.End();
                GL.PopMatrix();
            }

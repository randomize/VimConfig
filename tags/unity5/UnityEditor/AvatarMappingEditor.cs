    ...
    internal class AvatarMappingEditor : AvatarSubEditor
    {
        ...
        protected void ApplyTemplate()
        {
            ...
            if (template != null)
            {
                ...
                {
                    <ApplyTemplate>c__AnonStorey8D storeyd = new <ApplyTemplate>c__AnonStorey8D {
                        boneName = template.Find(this.m_Bones[i].humanBoneName)
                    };
                    if (storeyd.boneName.Length > 0)
                    {
                        Transform transform = base.modelBones.Keys.FirstOrDefault<Transform>(new Func<Transform, bool>(storeyd.<>m__19E));
            ...
            for (int i = 1; i < this.m_BodyPartToggle.Length; i++)
            {
                if (this.m_BodyPartToggle[i])
                {
                    ...
                    if (this.m_BodyPartFoldout[i])
                    {
                        ...
                        {
                            int index = this.m_BodyPartHumanBone[i][j];
                            if (index != -1)
                            {
        ...
        {
            <GetBoneState>c__AnonStorey8C storeyc = new <GetBoneState>c__AnonStorey8C();
            error = string.Empty;
            storeyc.bone = this.m_Bones[i];
            if (storeyc.bone.bone == null)
            {
            ...
            if ((i == 0) && (storeyc.bone.bone.parent == null))
            {
                error = storeyc.bone.messageName + " cannot be the root transform";
                return BoneState.InvalidHierarchy;
            }
            if ((wrapper.bone != null) && !storeyc.bone.bone.IsChildOf(wrapper.bone))
            {
                error = storeyc.bone.messageName + " is not a child of " + wrapper.messageName + ".";
                return BoneState.InvalidHierarchy;
            }
            if (((i != 0x17) && (wrapper.bone != null)) && (wrapper.bone != storeyc.bone.bone))
            {
                Vector3 vector = storeyc.bone.bone.position - wrapper.bone.position;
                if (vector.sqrMagnitude < Mathf.Epsilon)
                {
                    error = storeyc.bone.messageName + " has bone length of zero.";
                    return BoneState.BoneLenghtIsZero;
                }
            }
            if (this.m_Bones.Where<AvatarSetupTool.BoneWrapper>(new Func<AvatarSetupTool.BoneWrapper, bool>(storeyc.<>m__19D)).Count<AvatarSetupTool.BoneWrapper>() <= 1)
            {
                return BoneState.Valid;
            }
            error = storeyc.bone.messageName + " is also assigned to ";
            bool flag = true;
        ...
        protected void Init()
        {
            ...
            {
                this.m_IsBiped = AvatarBipedMapper.IsBiped(base.gameObject.transform);
        ...
        [CompilerGenerated]
        private sealed class <ApplyTemplate>c__AnonStorey8D
        {
            internal string boneName;

            internal bool <>m__19E(Transform f)
            {
        ...
        [CompilerGenerated]
        private sealed class <GetBoneState>c__AnonStorey8C
        {
            internal AvatarSetupTool.BoneWrapper bone;

            internal bool <>m__19D(AvatarSetupTool.BoneWrapper f)
            {

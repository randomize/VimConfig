namespace UnityEditor
{
    ...
    internal class DuckVolumeGUI : IAudioEffectPluginGUI
    {
        ...
        private static bool CurveDisplay(IAudioEffectPlugin plugin, Rect r0, ref float threshold, ref float ratio, ref float makeupGain, ref float attackTime, ref float releaseTime, ref float knee, float sidechainLevel, float outputLevel, float blend)
        {
            ...
            float num14;
            float num15;
            <CurveDisplay>c__AnonStorey53 storey = new <CurveDisplay>c__AnonStorey53 {
                blend = blend
            };
        ...
        Label_02F7:
            ...
            {
                <CurveDisplay>c__AnonStorey54 storey2 = new <CurveDisplay>c__AnonStorey54 {
                    <>f__ref$83 = storey
                };
                ...
                AudioCurveRendering.DrawFilledCurve(r, new AudioCurveRendering.AudioCurveAndColorEvaluator(storey2.<>m__89));
                if (dragtype == DragType.MakeupGain)
                {
                    AudioCurveRendering.DrawCurve(r, new AudioCurveRendering.AudioCurveEvaluator(storey2.<>m__8A), Color.white);
                }
                ...
                if (dragtype == DragType.Ratio)
                {
                    ...
                    Vector3[] vectorArray2 = new Vector3[] { new Vector3((r.x + num16) + r.width, (r.y + num18) - ((num19 * storey2.duckGradient) * r.width), 0f), new Vector3((r.x + num16) - r.width, (r.y + num18) + ((num19 * storey2.duckGradient) * r.width), 0f) };
        ...
        [CompilerGenerated]
        private sealed class <CurveDisplay>c__AnonStorey53
        {
        ...
        [CompilerGenerated]
        private sealed class <CurveDisplay>c__AnonStorey54
        {
            internal DuckVolumeGUI.<CurveDisplay>c__AnonStorey53 <>f__ref$83;
            ...
            internal float <>m__89(float x, out Color col)
            {
                float num = (x * this.<>f__ref$83.dbRange) + this.<>f__ref$83.dbMin;
                float num2 = num;
                float num3 = num - this.duckThreshold;
                col = DuckVolumeGUI.ScaleAlpha((this.duckSidechainLevel <= num) ? Color.grey : AudioCurveRendering.kAudioOrange, this.<>f__ref$83.blend);
                ...
                return (((2f * ((num2 + this.duckMakeupGain) - this.<>f__ref$83.dbMin)) / this.<>f__ref$83.dbRange) - 1f);
            }

            internal float <>m__8A(float x)
            {
                float num = (x * this.<>f__ref$83.dbRange) + this.<>f__ref$83.dbMin;
                float num2 = num;
                float num3 = num - this.duckThreshold;

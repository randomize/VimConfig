    ...
    internal sealed class _AndroidJNIHelper
    {
        ...
        public static IntPtr ConvertToJNIArray(Array array)
        {
            ...
            if (elementType == typeof(string))
            {
                ...
                for (int j = 0; j < num; j++)
                {
                    ptrArray[j] = AndroidJNISafe.NewStringUTF(strArray[j]);
                }
                IntPtr type = AndroidJNISafe.FindClass("java/lang/String");
                IntPtr ptr2 = AndroidJNISafe.ToObjectArray(ptrArray, type);
                AndroidJNISafe.DeleteLocalRef(type);
                return ptr2;
            }
            if (elementType != typeof(AndroidJavaObject))
            {
            ...
            IntPtr[] ptrArray2 = new IntPtr[length];
            ...
            for (int i = 0; i < length; i++)
            {
                if (objArray[i] != null)
                {
                    ptrArray2[i] = objArray[i].GetRawObject();
                ...
                {
                    ptrArray2[i] = IntPtr.Zero;
                }
            }
            IntPtr ptr6 = AndroidJNISafe.ToObjectArray(ptrArray2, zero);

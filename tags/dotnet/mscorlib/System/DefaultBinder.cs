namespace System
{
    ...
    internal class DefaultBinder : Binder
    {
        public override FieldInfo BindToField(BindingFlags bindingAttr, FieldInfo[] match, object value, CultureInfo cultureInfo)
        {
            ...
            {
                c = value.GetType();
                for (num = 0; num < match.Length; num++)
                {
                    Type fieldType = match[num].FieldType;
                    if (fieldType == c)
                    {
                        match[num2++] = match[num];
                    }
            ...
            {
                switch (FindMostSpecificField(match[index], match[num]))
                {
                    case 0:
                        flag = true;
                        break;
        ...
        private static int FindMostSpecific(ParameterInfo[] p1, int[] paramOrder1, Type paramArrayType1, ParameterInfo[] p2, int[] paramOrder2, Type paramArrayType2, Type[] types, object[] args)
        {
            ...
            if ((paramArrayType2 == null) || (paramArrayType1 != null))
            {
                ...
                for (int i = 0; i < types.Length; i++)
                {
                    ...
                    {
                        Type parameterType = p1[paramOrder1[i]].ParameterType;
        ...
        private static void ReorderParams(int[] paramOrder, object[] vars)
        {
            for (int i = 0; i < vars.Length; i++)
            {
                ...
                {
                    int num2 = paramOrder[paramOrder[i]];
                    object obj2 = vars[paramOrder[i]];
        ...
        public override MethodBase SelectMethod(BindingFlags bindingAttr, MethodBase[] match, Type[] types, ParameterModifier[] modifiers)
        {
            ...
            for (num = 0; num < match.Length; num++)
            {
                ParameterInfo[] parametersNoCopy = match[num].GetParametersNoCopy();
            ...
            {
                switch (FindMostSpecificMethod(match[index], numArray, null, match[num], numArray, null, types, null))
                {
                    case 0:
                        flag = true;
                        break;
        ...
        public override PropertyInfo SelectProperty(BindingFlags bindingAttr, PropertyInfo[] match, Type returnType, Type[] indexes, ParameterModifier[] modifiers)
        {
            ...
            for (num = 0; num < match.Length; num++)
            {
                if (indexes != null)
                {
                    ParameterInfo[] indexParameters = match[num].GetIndexParameters();
                ...
                if (returnType != null)
                {
                    if (match[num].PropertyType.IsPrimitive)
                    {
                        if ((returnType.UnderlyingSystemType is RuntimeType) && CanConvertPrimitive((RuntimeType) returnType.UnderlyingSystemType, (RuntimeType) match[num].PropertyType.UnderlyingSystemType))
                        {
                            goto Label_011E;
                        }
                        continue;
                    }
                    if (!match[num].PropertyType.IsAssignableFrom(returnType))
                    {
                        continue;
                    }
                }
            ...
            for (num = 1; num < num3; num++)
            {
                int num6 = FindMostSpecificType(match[num5].PropertyType, match[num].PropertyType, returnType);
                if ((num6 == 0) && (indexes != null))
                {
                    num6 = FindMostSpecific(match[num5].GetIndexParameters(), numArray, null, match[num].GetIndexParameters(), numArray, null, indexes, null);
                }
                if (num6 == 0)
                {
                    num6 = FindMostSpecificProperty(match[num5], match[num]);
                    if (num6 == 0)
                    {
                        flag = true;
                    }

namespace System
{
    ...
    internal static class DateTimeParse
    {
        ...
        private static bool AdjustTimeZoneToLocal(ref DateTimeResult result, bool bTimeOnly)
        {
            ...
            CurrentSystemTimeZone currentTimeZone = (CurrentSystemTimeZone) TimeZone.CurrentTimeZone;
            bool isAmbiguousLocalDst = false;
            if (ticks < 0xc92a69c000L)
            {
                ticks -= result.timeZoneOffset.Ticks;
                ticks += currentTimeZone.GetUtcOffset(bTimeOnly ? DateTime.Now : result.parsedDate).Ticks;
                if (ticks < 0L)
                {
                    ticks += 0xc92a69c000L;
                }
            ...
            else
            {
                ...
                {
                    ticks += currentTimeZone.GetUtcOffset(result.parsedDate).Ticks;
                }
                else
                {
                    ticks += currentTimeZone.GetUtcOffsetFromUniversalTime(new DateTime(ticks), ref isAmbiguousLocalDst);
                }
            }
            if ((ticks < 0L) || (ticks > 0x2bca2875f4373fffL))
            {

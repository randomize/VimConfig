    ...
    public abstract class Calendar : ICloneable
    {
        ...
        internal static int GetSystemTwoDigitYearSetting(int CalID, int defaultYearValue)
        {
            ...
            if (num < 0)
            {
                ...
                try
                {
                    key = Registry.CurrentUser.InternalOpenSubKey(@"Control Panel\International\Calendars\TwoDigitYearMax", false);
                }
                ...
                if (key != null)
                {
                    try
                    {
                        object obj2 = key.InternalGetValue(CalID.ToString(CultureInfo.InvariantCulture), null, false, false);
                        if (obj2 != null)
                        {
                            try
                            {
                                num = int.Parse(obj2.ToString(), CultureInfo.InvariantCulture);
                            }
                            catch (ArgumentException)
                            {
                            }

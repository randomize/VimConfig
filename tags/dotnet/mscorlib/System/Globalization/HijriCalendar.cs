namespace System.Globalization
{
    ...
    public class HijriCalendar : Calendar
    {
        ...
        private int GetAdvanceHijriDate()
        {
            ...
            try
            {
                key = Registry.CurrentUser.InternalOpenSubKey(m_InternationalRegKey, false);
            }
            ...
            if (key != null)
            {
                try
                {
                    object obj2 = key.InternalGetValue(m_HijriAdvanceRegKeyEntry, null, false, false);
                    if (obj2 == null)
                    {
                        return 0;
                    }
                    string strA = obj2.ToString();
                    if (string.Compare(strA, 0, m_HijriAdvanceRegKeyEntry, 0, m_HijriAdvanceRegKeyEntry.Length, StringComparison.OrdinalIgnoreCase) != 0)
                    {
                        return num;
                    }
                    if (strA.Length == m_HijriAdvanceRegKeyEntry.Length)
                    {
                        return -1;
                    }
                    strA = strA.Substring(m_HijriAdvanceRegKeyEntry.Length);
                    try
                    {

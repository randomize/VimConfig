namespace System.Globalization
{
    ...
    public sealed class IdnMapping
    {
        ...
        private bool m_bAllowUnassigned = false;
        private bool m_bUseStd3AsciiRules = false;
        ...
        public string GetUnicode(string ascii, int index, int count)
        {
            ...
            string unicode = this.punycode_decode(ascii);
        ...
        private string punycode_decode(string ascii)
        {
            ...
            if (ascii.Length > (0xff - (this.IsDot(ascii[ascii.Length - 1]) ? 0 : 1)))
            {
                throw new ArgumentException(Environment.GetResourceString("Argument_IdnBadNameSize", new object[] { 0xff - (this.IsDot(ascii[ascii.Length - 1]) ? 0 : 1) }), "ascii");
            }
            ...
            if (builder.Length > (0xff - (this.IsDot(builder[builder.Length - 1]) ? 0 : 1)))
            {
                throw new ArgumentException(Environment.GetResourceString("Argument_IdnBadNameSize", new object[] { 0xff - (this.IsDot(builder[builder.Length - 1]) ? 0 : 1) }), "ascii");
            }
        ...
        private string punycode_encode(string unicode)
        {
            ...
            {
                num = unicode.IndexOfAny(M_Dots, startIndex);
                if (num < 0)
                {
                    num = unicode.Length;
                }
                ...
                {
                    BidiCategory category2 = CharUnicodeInfo.GetBidiCategory(unicode, i);
                    ...
                    if (basic(unicode[i]))
                    {
                        builder.Append(encode_basic(unicode[i]));
                        num6++;
                    }
                ...
                int num7 = num6;
                if (num7 == (num - startIndex))
                {
                    builder.Remove(length, "xn--".Length);
                }
                else
                {
                    ...
                    while (num6 < (num - startIndex))
                    {
                        ...
                        while (num12 < num)
                        {
                            ...
                            num12 += this.IsSupplementary(cTest) ? 2 : 1;
                        }
                        delta += (num13 - num9) * ((num6 - num8) + 1);
                        num9 = num13;
                        for (num12 = startIndex; num12 < num; num12 += this.IsSupplementary(cTest) ? 2 : 1)
                        {
                            cTest = char.ConvertToUtf32(unicode, num12);
                            if (cTest < num9)
                            {
            ...
            if (builder.Length > (0xff - (this.IsDot(unicode[unicode.Length - 1]) ? 0 : 1)))
            {
                throw new ArgumentException(Environment.GetResourceString("Argument_IdnBadNameSize", new object[] { 0xff - (this.IsDot(unicode[unicode.Length - 1]) ? 0 : 1) }), "unicode");
            }
            return builder.ToString();
        }
        ...
        private bool ValidateStd3AndAscii(string unicode, bool bUseStd3, bool bCheckAscii)
        {
            ...
            if (unicode.Length > (0xff - (this.IsDot(unicode[unicode.Length - 1]) ? 0 : 1)))
            {
                throw new ArgumentException(Environment.GetResourceString("Argument_IdnBadNameSize", new object[] { 0xff - (this.IsDot(unicode[unicode.Length - 1]) ? 0 : 1) }), "unicode");
            }
